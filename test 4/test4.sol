...
*/
    ...
    function setUser(uint _a, string memory _b, address memory _c, uint _d, uint _e) public {
        u1.push(User(_a, _b, _c, _D, _e))

    
    function getUser(string memory _a) public view returns (uint, address memory, uint, uint)
        return Search[_a];
    }

            

    function changeEh(uint _a) public pure returns (uint) {
        if (_a = 10) {
            return 1 ether
        }
    }
