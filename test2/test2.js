...
//학생 추가 기능 - 특정 학생의 정보를 추가
    function addInfo(string memory _name, uint _number, uint _score, string memory _grade, string memory _class) public {
        return s1(string, uint, uint, string, string);
    }

//학생 조회 기능(1) - 특정 학생의 번호를 입력하면 그 학생 전체 정보를 반환
    function getInfo(uint _n) public view returns {student memory} {
        return s1[_n -1];
    }

//학생 조회 기능(2) - 특정 학생의 이름을 입력하면 그 학생 전체 정보를 반환
    function getInfo2(string memory _name) public view return {student memory} {
        return s1[string _name];
    }

//학생 점수 조회 기능 - 특정 학생의 이름을 입력하면 그 학생의 점수를 반환
        function getInfo3(string memory _name) public view return {student uint} {
        return s1[_n-1].score;
    }
